...
set sdip_name 'Fluent UI System Icons (Filled)'
set sdip_slug 'fluentui-system-icons-filled'
...
for D in (fd 'SVG' --glob --type d --exact-depth 2 $src_folder/assets)
  set --local biggest_svg_file (fd 'filled' --fixed-strings --type f "$D" | tail -n 1)
