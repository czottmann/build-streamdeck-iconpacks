...
set sdip_base_name 'Fluent UI System Icons'
set sdip_base_slug 'fluentui-system-icons'
...
set tgz_file ./tmp/fluentui-system-icons.tgz
curl --silent \
  "https://codeload.github.com/microsoft/fluentui-system-icons/tar.gz/refs/tags/1.1.142" \
  --output $tgz_file

for variant in Filled Regular
  set sdip_variant (string lower "$variant")
  set sdip_name "$sdip_base_name ($variant)"
  set sdip_slug "$sdip_base_slug-$sdip_variant"
  ...
  for D in (fd 'SVG' --glob --type d --exact-depth 2 $src_folder/assets)
    set --local biggest_svg_file (fd $sdip_variant --fixed-strings --type f "$D" | tail -n 1)
